...
const CLEAN_EVENT = {
  ...
  adapterName: "browser_gemini" as const,
  clientTimestamp: NOW,
  sequenceNumber: 0,
  adDecisionId: UUID,
  campaignId: UUID,
...
describe("Gemini adapter — privacy guard (browser forbidden fields)", () => {
  it("accepts a clean gemini event via validateBrowserEvent", () => {
...
describe("Gemini adapter — TelemetryEventSchema enforcement", () => {
  it("accepts browser_gemini as adapterName", () => {
    const result = TelemetryEventSchema.safeParse(CLEAN_EVENT);
    expect(result.success).toBe(true);
  });

  it("strips or rejects pageUrl in strict schema", () => {
    const result = TelemetryEventSchema.safeParse({ ...CLEAN_EVENT, pageUrl: "https://gemini.google.com/app/abc" });
    if (result.success) {
