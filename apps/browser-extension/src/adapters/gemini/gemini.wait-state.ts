/**
 * Gemini-specific wait-state detector.
 *
 * Uses MutationObserver on structural DOM selectors to detect when Gemini
 ...
 *
 * VERIFICATION STATUS: The underlying selectors (gemini.selectors.ts) are
 * UNVERIFIED against real gemini.google.com. This detector's logic itself
 * mirrors ChatGPTWaitStateDetector / ClaudeWaitStateDetector (proven,
 * verified/beta) exactly — only the selector source differs.
 */

import { GEMINI_PROCESSING_SELECTORS } from "./gemini.selectors.js";

export interface IGeminiWaitStateDetector {
  start(onStart: (startedAt: Date) => void, onEnd: () => void): void;
  stop(): void;
}

export class GeminiWaitStateDetector implements IGeminiWaitStateDetector {
  ...
  start(onStart: (startedAt: Date) => void, onEnd: () => void): void {
    ...
    const selectors = [...GEMINI_PROCESSING_SELECTORS];
