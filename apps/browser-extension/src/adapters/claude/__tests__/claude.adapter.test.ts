...
import type { ClaudeAdapterDeps } from "../claude.adapter.js";
...
function makeMockDetector(): IClaudeWaitStateDetector & {
  simulateStart: () => void;
  simulateEnd: () => void;
} {
...
function makeMockRenderer(): IClaudeRenderer & { _hasElement: boolean } {
...
function makeAdapter(hostnameOverride = "claude.ai", extra?: Partial<ClaudeAdapterDeps>) {
...
describe("ClaudeAdapter — identity", () => {
  it("has the correct adapter id", () => {
    ...
    expect(adapter.adapterId).toBe("browser_claude");
  });
...
describe("ClaudeAdapter — canActivate", () => {
  it("resolves true on claude.ai", async () => {
...
describe("ClaudeAdapter — wait-state handlers", () => {
  ...
  it("fires onWaitStateStart handler when detector fires", async () => {
    ...
    expect(handler.mock.calls[0]?.[0]).toMatchObject({ adapterId: "browser_claude" });
  });
...
describe("ClaudeAdapter — rendering", () => {
  let adapter: ClaudeAdapter;
...
describe("ClaudeAdapter — viewability", () => {
