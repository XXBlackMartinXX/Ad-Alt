...
const CLEAN_EVENT = {
  ...
  adapterName: "browser_claude" as const,
  clientTimestamp: NOW,
  sequenceNumber: 0,
  adDecisionId: UUID,
  campaignId: UUID,
...
describe("Claude adapter — privacy guard (browser forbidden fields)", () => {
  it("accepts a clean claude event via validateBrowserEvent", () => {
...
describe("Claude adapter — TelemetryEventSchema enforcement", () => {
  it("accepts browser_claude as adapterName", () => {
    const result = TelemetryEventSchema.safeParse(CLEAN_EVENT);
    expect(result.success).toBe(true);
  });

  it("strips or rejects pageUrl in strict schema", () => {
    const result = TelemetryEventSchema.safeParse({ ...CLEAN_EVENT, pageUrl: "https://claude.ai/chat/abc" });
    if (result.success) {
