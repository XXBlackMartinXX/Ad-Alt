/**
 * Smoke tests for the Gemini adapter integration.
 *
 ...
 *
 * SUPPORT STATUS: This proves Gemini at the fixture/unit-test level only. It
 * does NOT constitute a human-operated live-session verification against real
 * gemini.google.com -- see docs/internal-beta/platforms/GEMINI_BROWSER_VERIFICATION.md.
 * No real Gemini login or prompt entry occurs anywhere in this suite.
 */
...
async function openConfiguredPage(opts?: {
  ...
  return openFixturePage(extensionContext, fileServerPort, 'gemini-wait-state.html');
}

test('extension loads on Gemini fixture page without errors', async () => {
...
test('disabling only browser_gemini via disabledAdapters shows no banner', async () => {
  const page = await openConfiguredPage({ disabledAdapters: ['browser_gemini'] });
  await page.waitForTimeout(CONTENT_SCRIPT_SETTLE_MS);
...
test('ad-decision request and captured events for Gemini contain no forbidden fields', async () => {
  const page = await openConfiguredPage();
  ...
  for (const rawUrl of capturedDecisionUrls) {
    ...
    // adapterName must correctly identify Gemini, not ChatGPT/Claude.
    expect(url.searchParams.get('adapterName')).toBe('browser_gemini');
  }
  ...
  for (const event of events) {
    ...
    expect((event.body as Record<string, unknown>)['adapterName']).toBe('browser_gemini');
  }
