# Gemini Browser — Assisted Manual Live Verification Runbook

**Phase:** Safe No-Login First Live Verification Workflow
**Date:** 2026-07-03

> **Use this only if** `GEMINI_NO_LOGIN_LIVE_SMOKE_RUNBOOK.md` recorded
...
> could not reach a real wait-state because gemini.google.com requires a
> signed-in Google account to generate a response.
>
...
- You are using your own real Google account, at your own discretion and
...
Navigate to `https://gemini.google.com/` and sign in with your Google
account yourself, by hand, exactly as you normally would. Nothing in
this repo automates or assists this step.

### 3. Handle any CAPTCHA/security challenge manually

If Google's sign-in flow presents a CAPTCHA, 2FA prompt, or security
challenge, solve it yourself, normally. Nothing in this repo automates
or assists this step (CANARY 7).
...
While Gemini is generating (the real "Stop generating" button, or its
...
**If the real selector no longer matches** (Google's UI may have
changed since `gemini.selectors.ts`'s guess was written), the wait-state
...
button, positioned so it does not cover the prompt input or Gemini's
response.
...
page's own DOM, never Gemini's response text, never the conversation
...
Copy `GEMINI_ASSISTED_LIVE_VERIFICATION_RESULT_TEMPLATE.md` to
`GEMINI_ASSISTED_LIVE_VERIFICATION_RESULT_LOG.md` in this same directory
and fill in only the allowed fields.

---

## What must never be recorded

The prompt text and Gemini's response text are never written down
